...
double zmax, double tmax, double dt)
{
    ...
    for(j=0;j<parent.n2;++j)
  {
    ...
    {
      /* Original pwmig has a fatal error trap for this condition.
      In the mspass framework throwing an exception at this point is
  ...
  {
    /* I will leave this error message print here hoping it won't be common.
    To fix this requires adding an ErrorLogger attribute to the raygrid
    returned, which I don't care to do unless essential.   */
    cerr << "Warning:  Triming incident P wave grid from "
