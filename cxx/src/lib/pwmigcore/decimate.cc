...
using namespace std;
/*! \brief Decimate a GCLscalarfield3d.
...
GCLscalarfield3d *decimate(GCLscalarfield3d& g,int dec1, int dec2, int dec3)
{
  ...
  {
    stringstream ss;
    ss << "GLCscalarfield3d decimator:  decimation factor larger than grid dimension"<<endl
       << "Grid dimensions="<<n1<<"X"<<n2<<"X"<<n3<<endl
       << "Received decimator for each direction="<<dec1<<", "<<dec2<<", "<<dec3<<endl;
  ...
  GCLscalarfield3d *result=new GCLscalarfield3d(n1,n2,n3);;
  ...
  for(i=0,ii=0;i<g.n1 && ii<n1;i+=dec1,++ii)
  {
    for(j=0,jj=0;j<g.n2 && jj<n2;j+=dec2,++jj)
    {
      ...
      for(k=g.n3-1;kk>=0; k-=dec3,--kk)
      {
        ...
        result->val[ii][jj][kk]=g.val[i][j][k];
      }
    }
  }
  /* We clone other attributes of the parent grid.*/
  result->lat0=g.lat0;
  result->lon0=g.lon0;
  result->r0=g.r0;
