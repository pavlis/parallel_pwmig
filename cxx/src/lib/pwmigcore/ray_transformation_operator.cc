...
dmatrix Ray_Transformation_Operator::apply(dmatrix& in)
{
  ...
  {
    stringstream ss;
    ss << "Coding error:  Ray_Transformation_Operator has "
...
// Simple constuctor
Ray_Transformation_Operator::Ray_Transformation_Operator(const int np)
{
  npoints = np;
  for(int i=0;i<np;++i)
  {
  ...
  dmatrix& path,double azimuth, dmatrix& gamma_P)
{
  ...
  for(auto i=0;i<np;++i)
  {
    ...
    if(dnrm2(3,Tp,1)<PARALLEL_TEST)
    {
      ...
      if(dnrm2(3,Tp,1)<PARALLEL_TEST)
      {
        /* This is bad for when using this in a parallel environment but
        any messages like this should be rare so we will retain this rather
        than silence it. */
...
Ray_Transformation_Operator& Ray_Transformation_Operator::operator=(const Ray_Transformation_Operator& pat)
{
  if(this!=(&pat))
  {
    npoints=pat.npoints;
