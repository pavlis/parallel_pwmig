...
RegionalCoordinates::RegionalCoordinates(const double olat, const double olon,
        const double origr, const double oaz)
{
    lat0=olat;
    lon0=olon;
    r0=origr;
  ...
  const double lon, const double r) const
{
    ...
    // const_cast needed to deal with an ugly const collsion
    //
...
Cartesian_point RegionalCoordinates::cartesian(const Geographic_point gp) const
{
    return(this->cartesian(gp.lat,gp.lon,gp.r));
}
Geographic_point RegionalCoordinates::geographic
   (const double x, const double y, const double z) const
{
    ...
    xp[1]=y;
    xp[2]=z;
    Geographic_point gp(this->geographic(xp));
    return gp;
}
Geographic_point RegionalCoordinates::geographic(const double xin[3]) const
{
    Geographic_point  p;
    double dxp[3], x[3];
    int i;
...
Geographic_point RegionalCoordinates::geographic(const Cartesian_point cp) const
{
    double x[3];
    x[0]=cp.x1;
    x[1]=cp.x2;
...
Cartesian_point RegionalCoordinates::cartesian(const double x[3]) const
{
    Cartesian_point cp;
    cp.x1=x[0];
    cp.x2=x[1];
...
Geographic_point RegionalCoordinates::origin() const
{
    Geographic_point gp;
    gp.lat=lat0;
    gp.lon=lon0;
