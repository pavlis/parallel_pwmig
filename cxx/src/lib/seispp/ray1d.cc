...
RayPathSphere::RayPathSphere(const VelocityModel_1d& vmod,
		const double u, const double zmax, const double tmax,
		const double del, const string mode)
{
	list <double> ddlist;
	list <double> dzlist;
	list <double> dtlist;
	list <double>::iterator id, iz, it;
	double tnow,z;  // accumulated time and depth
...
double RayPathSphere::depth(const int ip) const
{
	const double R0=6378.17;
	if( (ip>=npts) || (ip<0) )
		throw MsPASSError("RayPathSphere::depth index out of range",ErrorSeverity::Invalid);
	else
		return(R0-r[ip]);
}
		...
		double theta, int ix1, int ix2, int ix3)
{
	...
	{
		stringstream ss;
		ss << "GCLgrid_Ray_project_down was passed an illegal index"<<endl
		  << "Received ix1="<<ix1<<" ix2="<<ix2<<" ix3="<<ix3<<endl
			<< "Must be nonegative values less than these values:  "
			<< grid.n1<<", "<<grid.n2<<", "<<grid.n3<<endl;
	...
	{
		stringstream ss;
		ss << "GCLgrid_Ray_project_down:  Given ray path does not reach requested depth="
		   <<depth<<endl
		   << "Ray turned at depth="<<path.r[0]-path.r[path.npts-1]<<endl;
		throw MsPASSError(ss.str(),ErrorSeverity::Invalid);
	}
		...
		double theta, int ix1, int ix2)
{
	...
	if(ix1>=grid.n1 || ix2>=grid.n2 || ix1<0 || ix2<0)
	{
		stringstream ss;
		ss << "GCLgrid_Ray_project_down was passed an illegal index"<<endl
		  << "Received ix1="<<ix1<<" ix2="<<ix2<<endl
			<< "Must be nonegative values less than these values:  "
			<< grid.n1<<", "<<grid.n2<<endl;
		throw MsPASSError(ss.str(),ErrorSeverity::Invalid);
	}
		...
		double theta, int ix1, int ix2, int ix3)
{
	...
	{
		stringstream ss;
		ss << "GCLgrid_Ray_project_up was passed an illegal index"<<endl
		  << "Received ix1="<<ix1<<" ix2="<<ix2<<" ix3="<<ix3<<endl
			<< "Must be nonegative values less than these values:  "
			<< grid.n1<<", "<<grid.n2<<", "<<grid.n3<<endl;
		throw MsPASSError(ss.str(),ErrorSeverity::Invalid);
	...
	{
		stringstream ss;
		ss << "GCLgrid_Ray_project_up:  Given ray path does not reach requested depth="
		   <<depth<<endl
		   << "Ray turned at depth="<<path.r[0]-path.r[path.npts-1]<<endl;
		throw MsPASSError(ss.str(),ErrorSeverity::Invalid);
	}
