...
# -*- coding: utf-8 -*-
"""
This is a new command line tool whose purpose is to take a table of 
data stored in a text file and extract some or all columns of the 
data in that table to a single MongoDB collection.   It allows 
filtering of the table based o set of unique keys to allow for 
things like views of Datascope tables where the same information is 
repeated in the table many times (e.g. css3.0 site table data).  

TODO:   complete this and documentation when the design is fleshed out 
further.
Usage line:
    
    dbview2collection mongodbname textfile [-pf pffile] [-v] [-has_header]
    
Most control should be in pffile.  the pffile by default should probably 
be the same as the textfile with .pf replacing any other file extension 
string.  

Created on Sat Oct  9 07:34:39 2021
...
def parse_attribute_name_tbl(pf,tag='attributes'):
    """
    This function will parse the pf file to extract a tbl with a specific 
